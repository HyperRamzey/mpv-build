...
CPU=nehalem
TUNE=nehalem
BUILD_DIR=/g/mpv-build/build-x64v2
PREFIX=/g/mpv-build/install-x64v2
LOG=/g/mpv-build/configure-x64v2.log

# --- Dependency paths (ALL external libs self-compiled per-target in G:\deps-build) ---
SRC=/g/mpv-build/mpv
FFMPEG_PREFIX=/g/ffmpeg-build/install-x64v2     # custom FFmpeg w/ dovi_split BSF (x64v2)
DEPS=/g/deps-build/deps-x64v2                   # self-built deps + static libplacebo (x64v2)
...
if [ ! -f "$DEPS/include/libplacebo/config.h" ]; then
  ...
  /g/mpv-build/build-libplacebo-x64v2.sh
fi
...
/clang64/bin/ninja -C "$BUILD_DIR" -j14 2>&1 | tee /g/mpv-build/make-x64v2.log | tail -10
echo "=== COMPILE EXIT: $? ==="
