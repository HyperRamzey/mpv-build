#!/bin/bash
# ==============================================================================
# mpv full build for target x64v2 (generic x86-64-v2 SSE4.2 CPU / any NVIDIA GPU — allcuda)
...
CPU=nehalem
TUNE=nehalem
BUILD_DIR=/g/media-build/mpv-build/build-x64v2
PREFIX=/g/media-build/mpv-build/install-x64v2
LOG=/g/media-build/mpv-build/configure-x64v2.log

# --- Dependency paths (ALL external libs self-compiled per-target in G:\media-build\deps-build) ---
SRC=/g/media-build/mpv-build/mpv
FFMPEG_PREFIX=/g/media-build/ffmpeg-build/install-x64v2     # custom FFmpeg w/ dovi_split BSF (x64v2)
...
if [ ! -f "$DEPS/include/libplacebo/config.h" ]; then
  echo "libplacebo x64v2 not found — building from source..."
  /g/media-build/mpv-build/build-libplacebo-x64v2.sh
fi
...
/clang64/bin/ninja -C "$BUILD_DIR" -j14 2>&1 | tee /g/media-build/mpv-build/make-x64v2.log | tail -10
echo "=== COMPILE EXIT: $? ==="
...
echo "=== x64v2 build complete: $PREFIX ==="
