...
target_env() {
	...
	zn2)
		ARCH=znver2
		TARGET_CPU="Zen2 + GTX 1650M (Pascal)"
		;;
	zn3)
		ARCH=znver3
		TARGET_CPU="Zen3 + RTX 5070 (Blackwell)"
		;;
	11700)
		ARCH=rocketlake
		TARGET_CPU="i7-11700 (RKL) + RTX 4080 (Ada)"
		;;
	3050)
		ARCH=znver2
		TARGET_CPU="Zen2 + RTX 3050M (Ampere)"
		;;
	14600)
		ARCH=raptorlake
		TARGET_CPU="i5-14600 (RPL) + RTX 50-series (Blackwell)"
		;;
	...
	x64v2)
		ARCH=nehalem
		TARGET_CPU="generic x86-64-v2 (SSE4.2) + any NVIDIA (allcuda)"
		;;
	x64v3)
		ARCH=haswell
		TARGET_CPU="generic x86-64-v3 (AVX2) + any NVIDIA (allcuda)"
		;;
	x64v4)
		ARCH=skylake-avx512
		TARGET_CPU="generic x86-64-v4 (AVX-512) + any NVIDIA (allcuda)"
		;;
...
meson_driver() { # args: srcdir builddir [meson options...]
	...
	meson setup "$bd" "$sd" --prefix="$PREFIX" --buildtype=release \
		...
		"${lto_args[@]}" \
		"$@" >&"$LOGF" || {
		echo "meson setup FAILED ($NAME)" >>"$LOGF"
		return 1
	}
	ninja -C "$bd" -j"${JOBS:-14}" >>"$LOGF" 2>&1 || {
		echo "ninja FAILED ($NAME)" >>"$LOGF"
		return 1
	}
	ninja -C "$bd" install >>"$LOGF" 2>&1 || {
		echo "install FAILED ($NAME)" >>"$LOGF"
		return 1
	}
}
...
cmake_driver() { # args: srcdir builddir [cmake options...]   (NO_CXX=1 skips CXX flags for pure-C projects)
	...
	cmake -S "$sd" -B "$bd" -G Ninja \
		...
		"$@" > >(tee -a "$LOGF") 2>&1 || {
		echo "cmake configure FAILED ($NAME)" >>"$LOGF"
		return 1
	}
	cmake --build "$bd" -j"${JOBS:-14}" >>"$LOGF" 2>&1 || {
		echo "cmake build FAILED ($NAME)" >>"$LOGF"
		return 1
	}
	cmake --install "$bd" >>"$LOGF" 2>&1 || {
		echo "cmake install FAILED ($NAME)" >>"$LOGF"
		return 1
	}
}
...
autotools_driver() { # args: srcdir [configure options...]
	...
	if [[ ! -f "$sd/configure" && ! -f "$sd/configure.ac" && ! -f "$sd/configure.in" ]]; then
		echo "ERROR ($NAME): no configure or configure.ac — cannot bootstrap" >>"$LOGF"
		return 1
	fi
	if [[ ! -f "$sd/configure" ]]; then
		(cd "$sd" && autoreconf -fi) >>"$LOGF" 2>&1 || {
			echo "autoreconf FAILED ($NAME)" >>"$LOGF"
			return 1
		}
	fi
	...
	(cd "$sd" && ./configure --prefix="$PREFIX" \
		...
		"$@") > >(tee -a "$LOGF") 2>&1 || {
		echo "configure FAILED ($NAME)" >>"$LOGF"
		return 1
	}
	make -C "$sd" -j"${JOBS:-14}" >>"$LOGF" 2>&1 || {
		echo "make FAILED ($NAME)" >>"$LOGF"
		return 1
	}
	make -C "$sd" install >>"$LOGF" 2>&1 || {
		echo "make install FAILED ($NAME)" >>"$LOGF"
		return 1
	}
}
