#!/bin/bash
# ==============================================================================
# mpv full build for target 3050 (znver2 CPU / RTX 3050M GPU)
...
# Outputs to a SEPARATE folder (install-3050) from the other targets.
# ==============================================================================
set -euo pipefail
...
CPU=znver2
TUNE=znver2
BUILD_DIR=/g/media-build/mpv-build/build-3050
PREFIX=/g/media-build/mpv-build/install-3050
LOG=/g/media-build/mpv-build/configure-3050.log

# --- Dependency paths (ALL external libs self-compiled per-target in G:\media-build\deps-build) ---
SRC=/g/media-build/mpv-build/mpv
FFMPEG_PREFIX=/g/media-build/ffmpeg-build/install-3050       # custom FFmpeg w/ dovi_split (3050)
DEPS=/g/media-build/deps-build/deps-3050                       # self-built deps + libplacebo (3050)
...
echo "=== mpv build for target 3050 (znver2) ($CPU) ==="

# --- Ensure libplacebo is built (needed for DoVi FEL) ---
if [ ! -f "$DEPS/include/libplacebo/config.h" ]; then
  echo "libplacebo 3050 not found — building from source..."
  /g/media-build/mpv-build/build-libplacebo-3050.sh
fi
...
/clang64/bin/ninja -C "$BUILD_DIR" -j14 2>&1 | tee /g/media-build/mpv-build/make-3050.log | tail -10
echo "=== COMPILE EXIT: $? ==="
...
echo "=== 3050 (znver2) build complete: $PREFIX ==="
