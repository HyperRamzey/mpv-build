...
CPU=znver2
TUNE=znver2
BUILD_DIR=/g/mpv-build/build-3050
PREFIX=/g/mpv-build/install-3050
LOG=/g/mpv-build/configure-3050.log

# --- Dependency paths (ALL external libs self-compiled per-target in G:\deps-build) ---
SRC=/g/mpv-build/mpv
FFMPEG_PREFIX=/g/ffmpeg-build/install-3050       # custom FFmpeg w/ dovi_split (3050)
DEPS=/g/deps-build/deps-3050                       # self-built deps + libplacebo (3050)
...
if [ ! -f "$DEPS/include/libplacebo/config.h" ]; then
  ...
  /g/mpv-build/build-libplacebo-3050.sh
fi
...
/clang64/bin/ninja -C "$BUILD_DIR" -j14 2>&1 | tee /g/mpv-build/make-3050.log | tail -10
echo "=== COMPILE EXIT: $? ==="
