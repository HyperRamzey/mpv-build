...
CPU=skylake-avx512
TUNE=skylake-avx512
BUILD_DIR=/g/media-build/mpv-build/build-x64v4
PREFIX=/g/media-build/mpv-build/install-x64v4
LOG=/g/media-build/mpv-build/configure-x64v4.log

# --- Dependency paths (ALL external libs self-compiled per-target in G:\media-build\deps-build) ---
SRC=/g/media-build/mpv-build/mpv
FFMPEG_PREFIX=/g/media-build/ffmpeg-build/install-x64v4     # custom FFmpeg w/ dovi_split BSF (x64v4)
DEPS=/g/media-build/deps-build/deps-x64v4                   # self-built deps + static libplacebo (x64v4)
...
if [ ! -f "$DEPS/include/libplacebo/config.h" ]; then
  ...
  /g/media-build/mpv-build/build-libplacebo-x64v4.sh
fi
...
/clang64/bin/ninja -C "$BUILD_DIR" -j14 2>&1 | tee /g/media-build/mpv-build/make-x64v4.log | tail -10
echo "=== COMPILE EXIT: $? ==="
