#!/bin/bash
# FFmpeg build for x64v2 (generic x86-64 ISA level / ALL NVIDIA GPUs — allcuda sm_75 PTX 6.3) — clang 22 / CLANG64
...
PREFIX=/g/media-build/ffmpeg-build/install-x64v2
LOG=/g/media-build/ffmpeg-build/configure-x64v2.log
...
make LD="$DEPS/wat4ff_ld" WAT4FF_TRUELD=clang -j14 2>&1 | tee /g/media-build/ffmpeg-build/make-x64v2.log | grep -E '^(CC|CXX|LD|CUDA|error|Error|warning:)' | tail -5
echo "=== MAKE EXIT: $? ==="
...
echo "=== FFmpeg x64v2 installed to $PREFIX ==="
