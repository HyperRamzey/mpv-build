#!/bin/bash
# FFmpeg build for Zen2 (znver2 / RTX 3050M sm_86) — clang 22 / CLANG64
...
PREFIX=/g/media-build/ffmpeg-build/install-3050
LOG=/g/media-build/ffmpeg-build/configure-3050.log
DEPS=/g/media-build/deps-build/deps-3050
cd "$SRC"
...
OPT="-O3 -march=znver2 -mtune=znver2 -mprefer-vector-width=256 -fvectorize -fslp-vectorize -funroll-loops -fomit-frame-pointer -fstrict-aliasing -fno-trapping-math"

# CUDA nvcc flags: sm_86 (Ampere RTX 3050M), clang NVPTX backend
NVCCFLAGS="--cuda-gpu-arch=sm_86 -Xclang -target-feature -Xclang +ptx87 -O3"
...
echo "=== FFmpeg configure (znver2 / sm_86) ==="
./configure \
	--prefix="$PREFIX" \
...
make LD="$DEPS/wat4ff_ld" WAT4FF_TRUELD=clang -j14 2>&1 | tee /g/media-build/ffmpeg-build/make-3050.log | grep -E '^(CC|CXX|LD|CUDA|error|Error|warning:)' | tail -5
echo "=== MAKE EXIT: $? ==="
...
echo "=== FFmpeg 3050 installed to $PREFIX ==="
