...
PREFIX=/g/ffmpeg-build/install-14600
LOG=/g/ffmpeg-build/configure-14600.log
DEPS=/g/deps-build/deps-14600
cd "$SRC"
...
make LD="$DEPS/wat4ff_ld" WAT4FF_TRUELD=clang -j14 2>&1 | tee /g/ffmpeg-build/make-14600.log | grep -E '^(CC|CXX|LD|CUDA|error|Error|warning:)' | tail -5
echo "=== MAKE EXIT: $? ==="
