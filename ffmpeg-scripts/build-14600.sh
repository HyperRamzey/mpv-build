#!/bin/bash
# FFmpeg build for 14600 (raptorlake / RTX 50-series sm_120a) — clang 22 / CLANG64
...
PREFIX=/g/media-build/ffmpeg-build/install-14600
LOG=/g/media-build/ffmpeg-build/configure-14600.log
DEPS=/g/media-build/deps-build/deps-14600
cd "$SRC"
...
# Self-built deps FIRST; /clang64 only for system runtime bits
...
OPT="-O3 -march=raptorlake -mtune=raptorlake -mprefer-vector-width=256 -fvectorize -fslp-vectorize -funroll-loops -fomit-frame-pointer -fstrict-aliasing -fno-trapping-math"

# CUDA nvcc flags: sm_120a (Blackwell RTX 50-series), clang NVPTX backend
NVCCFLAGS="--cuda-gpu-arch=sm_120a -Xclang -target-feature -Xclang +ptx87 -O3"
...
echo "=== FFmpeg configure (raptorlake / sm_120a) ==="
./configure \
	--prefix="$PREFIX" \
...
make LD="$DEPS/wat4ff_ld" WAT4FF_TRUELD=clang -j14 2>&1 | tee /g/media-build/ffmpeg-build/make-14600.log | grep -E '^(CC|CXX|LD|CUDA|error|Error|warning:)' | tail -5
echo "=== MAKE EXIT: $? ==="
...
echo "=== FFmpeg 14600 installed to $PREFIX ==="
