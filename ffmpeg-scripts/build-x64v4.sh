...
PREFIX=/g/ffmpeg-build/install-x64v4
LOG=/g/ffmpeg-build/configure-x64v4.log
DEPS=/g/deps-build/deps-x64v4
cd "$SRC"
...
make LD="$DEPS/wat4ff_ld" WAT4FF_TRUELD=clang -j14 2>&1 | tee /g/ffmpeg-build/make-x64v4.log | grep -E '^(CC|CXX|LD|CUDA|error|Error|warning:)' | tail -5
echo "=== MAKE EXIT: $? ==="
