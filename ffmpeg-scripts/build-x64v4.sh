#!/bin/bash
# FFmpeg build for x64v4 (skylake-avx512 / allcuda — runs on every NVIDIA GPU) — clang 22 / CLANG64
...
PREFIX=/g/media-build/ffmpeg-build/install-x64v4
LOG=/g/media-build/ffmpeg-build/configure-x64v4.log
DEPS=/g/media-build/deps-build/deps-x64v4
cd "$SRC"
...
OPT="-O3 -march=skylake-avx512 -mtune=skylake-avx512 -mprefer-vector-width=256 -fvectorize -fslp-vectorize -funroll-loops -fomit-frame-pointer -fstrict-aliasing -fno-trapping-math"
...
echo "=== FFmpeg configure (skylake-avx512 / sm_120a) ==="
./configure \
	--prefix="$PREFIX" \
...
make LD="$DEPS/wat4ff_ld" WAT4FF_TRUELD=clang -j14 2>&1 | tee /g/media-build/ffmpeg-build/make-x64v4.log | grep -E '^(CC|CXX|LD|CUDA|error|Error|warning:)' | tail -5
echo "=== MAKE EXIT: $? ==="
...
echo "=== FFmpeg x64v4 installed to $PREFIX ==="
